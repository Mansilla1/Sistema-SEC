var App = function() {
    ...
    var handleSliders = function() {
        ...
        $("#slider").slider({value: 5, slide: repositionTooltip, stop: repositionTooltip, min: 0, max: 10});
    ...
    var graficos = function() {
        ...
        function diurno() {
            ...
            var myChart = new xChart('bar', data, '#diurno');
        }
        ...
        function vespertino() {
            ...
            var myChart = new xChart('bar', data, '#vespertino');
        }
        ...
        function advance() {
            ...
            var myChart = new xChart('bar', data, '#advance');
        }
...
/*-----------------------------------------------------------------------------------*/
    /*	Hubspot messenger
     /*-----------------------------------------------------------------------------------*/
    var mihandleMessenger = function() {
    ...
    var MihandleSliders = function() {
        ...
        $("#slider1").slider({value: 5, slide: repositionTooltip, stop: repositionTooltip, min: 0, max: 10});
        $("#slider2").slider({value: 5, slide: repositionTooltip, stop: repositionTooltip, min: 0, max: 10});
        $("#slider3").slider({value: 5, slide: repositionTooltip, stop: repositionTooltip, min: 0, max: 10});
        $("#slider4").slider({value: 5, slide: repositionTooltip, stop: repositionTooltip, min: 0, max: 10});
        $("#slider5").slider({value: 5, slide: repositionTooltip, stop: repositionTooltip, min: 0, max: 10});
        ...
        $("#slider3 .ui-slider-handle:first").tooltip({title: $("#slider3").slider("value"), trigger: "manual"}).tooltip("show");
        $("#slider4 .ui-slider-handle:first").tooltip({title: $("#slider4").slider("value"), trigger: "manual"}).tooltip("show");
        $("#slider5 .ui-slider-handle:first").tooltip({title: $("#slider5").slider("value"), trigger: "manual"}).tooltip("show");

        $("#slider-horizontal-multiple1 > span").each(function() {
            var a = parseInt($(this).text(), 10);
            $(this).empty().slider({
                value: 5, slide: repositionTooltip, stop: repositionTooltip, min: 0, max: 10}
            )
        }
        );
    ...
    var MihandleRaty = function() {
       /* $.fn.raty.defaults.path = 'js/jquery-raty/img';
        $('#number-demo1').raty({
            number: 7,
            scoreName: "score1",
            score: 0,
            hints: ['Muy mal', 'Mal', 'Deficiente', 'Regular', 'Bien', 'Muy bien', 'Excelente']
        });
        $('#number-demo2').raty({
            number: 7,
            scoreName: "score2",
...
score: 0,
                ...
                starHalf: 'star-half-big.png',
                iconRange: [
                    {range: 1, on: 'star-on-big1.svg'},
                    {range: 2, on: 'star-on-big2.svg'},
                    {range: 3, on: 'star-on-big3.svg'},
                    {range: 4, on: 'star-on-big4.svg'},
                    {range: 5, on: 'star-on-big5.svg'},
                    {range: 6, on: 'star-on-big6.svg'},
                    {range: 7, on: 'star-on-big7.svg'}
                ],
                starOff: 'star-on-big-off.svg',
                starOn: 'star-on-big.png'
            }
            );
            $('#size-demo2').raty({
...
var handleProfileEdit = function() {
    ...
    return {
        ...
        init: function() {
            ...
            if (App.isPage("mantenedor_pregunta")) {
            ...
            /*Funciones que agrego*/
            handleBoxSortable();
            if (App.isPage("responder_encuesta")) {
                handleBootbox(); //Function to display Date Timepicker
                handleMagicSuggest(); //Function to display autosuggest
                handleDateColorpicker(); //Function to handle date and time picker
                handleRaty(); //To show star ratings
